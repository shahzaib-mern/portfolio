...
//  Enter your email → they send you the key
//  It's a PUBLIC key — safe to keep in frontend
// ─────────────────────────────────────────
...
async function sendEmail({ name, email, message }) {
  const response = await fetch("https://api.web3forms.com/submit", {
    ...
    body: JSON.stringify({
      access_key: "8177812e-d882-4645-acd8-89f7bf8ff1e3",
      name,
      email,
      message,
      subject: `New Inquiry from ${name} — Shahzaib's Portfolio`,
      from_name: "Shahzaib Portfolio",
      // Redirects the reply-to so you can reply directly to the client
      replyto: email,
    }),
  });
    ...
    >
      <div className="section-container relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-16 items-start">
          ...
          {/* ── Info Side ── */}
          <div
            className={`lg:col-span-5 flex flex-col gap-12 transition-all duration-1000 ${vis ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-12"
              }`}
          >
            <div>
          ...
          {/* ── Form Side ── */}
          <div
            className={`lg:col-span-7 transition-all duration-1000 delay-200 ${vis ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
          >
